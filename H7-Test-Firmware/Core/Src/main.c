...
//#define ADCRESULTSDMA3_LENGTH 4
volatile uint16_t adcResultsDMA[ADCRESULTSDMA_LENGTH]; // array to store results of Internal ADC Poll (not sure if it has to be "volatile") set for ADC_1
//volatile uint16_t adcResultsDMA3[ADCRESULTSDMA3_LENGTH]; //array for ADC channel 3
    ...
    { // check pin state
      ...
      if (HK_ON)
      {
        if (hk_counter == HK_CADENCE)
        {
          ...
          //atm --> IT IS NOT therefore it is not the ADCs = 0
          adcResultsDMA[0] = 123;
...
static void MX_MDMA_Init(void)
{
  ...
  hmdma_mdma_channel40_dma1_stream1_tc_0.Init.BufferTransferLength = 2;
  ...
  if (HAL_MDMA_ConfigPostRequestMask(&hmdma_mdma_channel40_dma1_stream1_tc_0, 0, 0) != HAL_OK)
  {
    Error_Handler();
  }
