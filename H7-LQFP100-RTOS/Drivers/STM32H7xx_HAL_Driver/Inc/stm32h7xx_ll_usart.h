...
__STATIC_INLINE void LL_USART_ConfigFIFOsThreshold(USART_TypeDef *USARTx,
		uint32_t TXThreshold, uint32_t RXThreshold) {
...
__STATIC_INLINE void LL_USART_SetBaudRate(USART_TypeDef *USARTx,
		uint32_t PeriphClk, uint32_t PrescalerValue, uint32_t OverSampling,
		uint32_t BaudRate) {
	uint32_t usartdiv;
	uint32_t brrtemp;
	...
	if (PrescalerValue > LL_USART_PRESCALER_DIV256) {
		...
		usartdiv = (uint16_t) (__LL_USART_DIV_SAMPLING8(PeriphClk,
				(uint8_t )PrescalerValue, BaudRate));
...
__STATIC_INLINE uint32_t LL_USART_GetBaudRate(const USART_TypeDef *USARTx,
	...
	uint32_t periphclkpresc = (uint32_t) (PeriphClk
			/ (USART_PRESCALER_TAB[(uint8_t) PrescalerValue]));
 ...
 * @retval State of bit (1 or 0).
 */
__STATIC_INLINE uint32_t LL_USART_IsEnabledSPISlaveSelect(
		const USART_TypeDef *USARTx) {
 ...
 */
__STATIC_INLINE uint32_t LL_USART_GetDEDeassertionTime(
		const USART_TypeDef *USARTx) {
 ...
 * @retval State of bit (1 or 0).
 */
__STATIC_INLINE uint32_t LL_USART_IsActiveFlag_RXNE_RXFNE(
		const USART_TypeDef *USARTx) {
	return ((READ_BIT(USARTx->ISR, USART_ISR_RXNE_RXFNE)
 ...
 * @retval State of bit (1 or 0).
 */
__STATIC_INLINE uint32_t LL_USART_IsActiveFlag_TXE_TXFNF(
		const USART_TypeDef *USARTx) {
 ...
 * @retval State of bit (1 or 0).
 */
__STATIC_INLINE uint32_t LL_USART_IsActiveFlag_TCBGT(
		const USART_TypeDef *USARTx) {
 ...
 * @retval State of bit (1 or 0).
 */
__STATIC_INLINE uint32_t LL_USART_IsEnabledIT_RXNE_RXFNE(
		const USART_TypeDef *USARTx) {
 ...
 * @retval State of bit (1 or 0).
 */
__STATIC_INLINE uint32_t LL_USART_IsEnabledIT_TXE_TXFNF(
		const USART_TypeDef *USARTx) {
 ...
 * @retval State of bit (1 or 0).
 */
__STATIC_INLINE uint32_t LL_USART_IsEnabledDMADeactOnRxErr(
		const USART_TypeDef *USARTx) {
