...
HAL_StatusTypeDef HAL_ADCEx_LinearCalibration_GetValue(ADC_HandleTypeDef *hadc,
	...
	if (tmp_hal_status == HAL_OK) {
		...
		for (cnt = ADC_LINEAR_CALIB_REG_COUNT; cnt > 0UL; cnt--) {
			LinearCalib_Buffer[cnt - 1U] = LL_ADC_GetCalibrationLinearFactor(
					hadc->Instance,
					ADC_CR_LINCALRDYW6 >> (ADC_LINEAR_CALIB_REG_COUNT - cnt));
		}
...
HAL_StatusTypeDef HAL_ADCEx_Calibration_SetValue(ADC_HandleTypeDef *hadc,
  ...
  else
  {
	...
	if ((LL_ADC_IsEnabled(hadc->Instance) != 0UL)
		...
		LL_ADC_SetCalibrationOffsetFactor(hadc->Instance, SingleDiff,
				CalibrationFactor);
	} else {
...
HAL_StatusTypeDef HAL_ADCEx_LinearCalibration_SetValue(ADC_HandleTypeDef *hadc,
	...
	if (HAL_IS_BIT_CLR(hadc->Instance->CR, ADC_CR_ADVREGEN)) {
		...
		wait_loop_index = ((ADC_STAB_DELAY_US / 10UL)
	...
	if (LL_ADC_IsEnabled(hadc->Instance) == 0UL) /* Enable the ADC if it is disabled */
	{
		if (ADC_Enable(hadc) != HAL_OK) {
			...
			for (cnt = ADC_LINEAR_CALIB_REG_COUNT; cnt > 0UL; cnt--) {
				LL_ADC_SetCalibrationLinearFactor(hadc->Instance,
						ADC_CR_LINCALRDYW6
								>> (ADC_LINEAR_CALIB_REG_COUNT - cnt),
						LinearCalib_Buffer[cnt - 1U]);
			}
 ...
 * @retval HAL state
 */
HAL_StatusTypeDef HAL_ADCEx_LinearCalibration_FactorLoad(
	...
	for (cnt = 0UL; cnt < ADC_LINEAR_CALIB_REG_COUNT; cnt++) {
		LinearCalib_Buffer[cnt] = *(uint32_t*) (ADC_LINEAR_CALIB_REG_1_ADDR
				+ FactorOffset + cnt);
	}
	if (HAL_ADCEx_LinearCalibration_SetValue(hadc,
			(uint32_t*) LinearCalib_Buffer) != HAL_OK) {
		tmp_hal_status = HAL_ERROR;
	}
...
HAL_StatusTypeDef HAL_ADCEx_InjectedStart(ADC_HandleTypeDef *hadc) {
	...
	if (LL_ADC_INJ_IsConversionOngoing(hadc->Instance) != 0UL) {
		...
		/* In case of software trigger detection enabled, JQDIS must be set
		 ...
		 the queue is empty */
		tmp_config_injected_queue = READ_BIT(hadc->Instance->CFGR,
				ADC_CFGR_JQDIS);
		...
		if (tmp_hal_status == HAL_OK) {
			...
			if ((__LL_ADC_MULTI_INSTANCE_MASTER(hadc->Instance)
				...
				if (LL_ADC_INJ_GetTrigAuto(
						hadc->Instance) == LL_ADC_INJ_TRIG_INDEPENDENT) {
...
HAL_StatusTypeDef HAL_ADCEx_InjectedPollForConversion(ADC_HandleTypeDef *hadc,
	...
	tmp_adc_inj_is_trigger_source_sw_start = LL_ADC_INJ_IsTriggerSourceSWStart(
			hadc->Instance);
	tmp_adc_reg_is_trigger_source_sw_start = LL_ADC_REG_IsTriggerSourceSWStart(
			hadc->Instance);
...
HAL_StatusTypeDef HAL_ADCEx_InjectedStop_IT(ADC_HandleTypeDef *hadc) {
	...
	if (tmp_hal_status == HAL_OK) {
		...
		__HAL_ADC_DISABLE_IT(hadc,
				(ADC_IT_JEOC | ADC_IT_JEOS | ADC_FLAG_JQOVF));
	...
	if (LL_ADC_REG_IsConversionOngoing(hadc->Instance) != 0UL) {
		...
		if (tmp_hal_status == HAL_OK) {
			...
			if (__LL_ADC_MULTI_INSTANCE_MASTER(hadc->Instance)
				...
				tmp_hal_status = HAL_DMA_Start_IT(hadc->DMA_Handle,
						(uint32_t) &tmpADC_Common->CDR, (uint32_t) pData,
						Length);
...
HAL_StatusTypeDef HAL_ADCEx_RegularStop_DMA(ADC_HandleTypeDef *hadc) {
	...
	if (tmp_hal_status == HAL_OK) {
		...
		MODIFY_REG(hadc->Instance->CFGR, ADC_CFGR_DMNGT_0 | ADC_CFGR_DMNGT_1,
				0UL);
	...
	if (tmp_hal_status == HAL_OK) {
		...
		if (tmp_hal_status == HAL_OK) {
			if (LL_ADC_INJ_IsConversionOngoing(hadc->Instance) == 0UL) {
				tmp_hal_status = ADC_Disable(hadc);
				if (tmp_hal_status == HAL_OK) {
					if (LL_ADC_INJ_IsConversionOngoing(
							(&tmphadcSlave)->Instance) == 0UL) {
...
HAL_StatusTypeDef HAL_ADCEx_InjectedConfigChannel(ADC_HandleTypeDef *hadc,
	...
	assert_param(
			IS_ADC_SINGLE_DIFFERENTIAL(sConfigInjected->InjectedSingleDiff));
  ...
  if (hadc->Instance == ADC3)
  {
	...
	if (hadc->Init.OversamplingMode == ENABLE) {
		assert_param(
				IS_ADC_RANGE(ADC_GET_RESOLUTION(hadc), sConfigInjected->InjectedOffset / (hadc->Init.Oversampling.Ratio + 1U)));
	} else {
		assert_param(
				IS_ADC_RANGE(ADC_GET_RESOLUTION(hadc), sConfigInjected->InjectedOffset));
	}
  ...
  if (hadc->Instance == ADC3)
  {
	...
	assert_param(
			!((hadc->Init.DiscontinuousConvMode == ENABLE) && (sConfigInjected->AutoInjectedConv == ENABLE)));
	...
	if (sConfigInjected->InjectedSingleDiff != ADC_DIFFERENTIAL_ENDED) {
		...
		if (hadc->Instance == ADC1) {
			assert_param(
					IS_ADC1_DIFF_CHANNEL(sConfigInjected->InjectedChannel));
		}
		if (hadc->Instance == ADC2) {
			assert_param(
					IS_ADC2_DIFF_CHANNEL(sConfigInjected->InjectedChannel));
		}
#if defined (ADC3)
		if (hadc->Instance == ADC3) {
			assert_param(
					IS_ADC3_DIFF_CHANNEL(sConfigInjected->InjectedChannel));
		}
#endif
	}
...
#endif
			assert_param(
					IS_ADC_RIGHT_BIT_SHIFT(sConfigInjected->InjecOversampling.RightBitShift));
      ...
      else
      {
        MODIFY_REG(hadc->Instance->CFGR2,
			...
			MODIFY_REG(hadc->Instance->CFGR2,
					...
					ADC_CFGR2_JOVSE | ((sConfigInjected->InjecOversampling.Ratio - 1UL) << ADC_CFGR2_OVSR_Pos) | sConfigInjected->InjecOversampling.RightBitShift);
#endif
		} else {
        ...
        LL_ADC_SetOffsetSaturation(hadc->Instance, sConfigInjected->InjectedOffsetNumber, (sConfigInjected->InjectedOffsetSaturation == ENABLE) ? LL_ADC_OFFSET_SATURATION_ENABLE : LL_ADC_OFFSET_SATURATION_DISABLE);
...
#endif /* ADC_VER_V5_V90 */
			{
				...
				LL_ADC_SetOffsetSignedSaturation(hadc->Instance,
						sConfigInjected->InjectedOffsetNumber,
						(sConfigInjected->InjectedOffsetSignedSaturation
								== ENABLE) ?
