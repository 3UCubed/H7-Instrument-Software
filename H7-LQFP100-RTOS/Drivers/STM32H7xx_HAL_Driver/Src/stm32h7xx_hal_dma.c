...
HAL_StatusTypeDef HAL_DMA_PollForTransfer(DMA_HandleTypeDef *hdma,
	...
	if (IS_DMA_STREAM_INSTANCE(hdma->Instance) != 0U) /* DMA1 or DMA2 instance */
	{
		...
		if ((((DMA_Stream_TypeDef*) hdma->Instance)->CR & DMA_SxCR_CIRC)
				!= 0U) {
	...
	} else /* BDMA channel */
	{
		...
		if ((((BDMA_Channel_TypeDef*) hdma->Instance)->CCR & BDMA_CCR_CIRC)
				!= 0U) {
			hdma->ErrorCode = HAL_DMA_ERROR_NOT_SUPPORTED;
			return HAL_ERROR;
		}
	...
	while (((*isr_reg) & cpltlevel_mask) == 0U) {
		if (IS_DMA_STREAM_INSTANCE(hdma->Instance) != 0U) /* DMA1 or DMA2 instance */
		{
			if (((*isr_reg) & (DMA_FLAG_FEIF0_4 << (hdma->StreamIndex & 0x1FU)))
					!= 0U) {
				/* Update error code */
			...
			if (((*isr_reg) & (DMA_FLAG_DMEIF0_4 << (hdma->StreamIndex & 0x1FU)))
					!= 0U) {
				/* Update error code */
			...
			if (((*isr_reg) & (DMA_FLAG_TEIF0_4 << (hdma->StreamIndex & 0x1FU)))
					!= 0U) {
				/* Update error code */
		...
		{
			if (((*isr_reg) & (BDMA_FLAG_TE0 << (hdma->StreamIndex & 0x1FU)))
					!= 0U) {
	...
	if (CompleteLevel == HAL_DMA_FULL_TRANSFER) {
		...
		if (IS_DMA_STREAM_INSTANCE(hdma->Instance) != 0U) /* DMA1 or DMA2 instance */
		{
			(*ifcr_reg) = (DMA_FLAG_HTIF0_4 | DMA_FLAG_TCIF0_4)
					<< (hdma->StreamIndex & 0x1FU);
		} else /* BDMA channel */
		{
