	...
	if (hdma->Init.Direction == DMA_MEMORY_TO_MEMORY) {
		...
		if (HAL_DMA_STATE_READY == hdma->State) {
			...
			if (IS_DMA_STREAM_INSTANCE(hdma->Instance) != 0U) /* DMA1 or DMA2 instance */
			{
				...
				ifcRegister_Base =
						(uint32_t*) ((uint32_t) (hdma->StreamBaseAddress + 8U));
			...
			{
				/* Enable the Double buffer mode */
				((BDMA_Channel_TypeDef*) hdma->Instance)->CCR |= (BDMA_CCR_DBM
						| BDMA_CCR_CIRC);

				/* Configure DMA Stream destination address */
				((BDMA_Channel_TypeDef*) hdma->Instance)->CM1AR =
						SecondMemAddress;

				/* Calculate the interrupt clear flag register (IFCR) base address  */
				ifcRegister_Base =
						(uint32_t*) ((uint32_t) (hdma->StreamBaseAddress + 4U));

				/* Clear all flags */
				*ifcRegister_Base = (BDMA_ISR_GIF0)
...
HAL_StatusTypeDef HAL_DMAEx_MultiBufferStart_IT(DMA_HandleTypeDef *hdma,
	...
	if (HAL_DMA_STATE_READY == hdma->State) {
		...
		if (IS_DMA_STREAM_INSTANCE(hdma->Instance) != 0U) /* DMA1 or DMA2 instance */
		{
			...
			/* Calculate the interrupt clear flag register (IFCR) base address  */
			ifcRegister_Base = (uint32_t*) ((uint32_t) (hdma->StreamBaseAddress
					+ 8U));
...
HAL_StatusTypeDef HAL_DMAEx_ConfigMuxSync(DMA_HandleTypeDef *hdma,
	...
	if (pSyncConfig->SyncEnable == ENABLE) {
		...
		{
			assert_param(
					IS_DMA_DMAMUX_SYNC_SIGNAL_ID(pSyncConfig->SyncSignalID));
		} else {
			assert_param(
					IS_BDMA_DMAMUX_SYNC_SIGNAL_ID(pSyncConfig->SyncSignalID));
		}
	...
	if (hdma->State == HAL_DMA_STATE_READY) {
		...
		MODIFY_REG(hdma->DMAmuxChannel->CCR, (~DMAMUX_CxCR_DMAREQ_ID),
				(syncSignalID << DMAMUX_CxCR_SYNC_ID_Pos) | ((pSyncConfig->RequestNumber - 1U) << DMAMUX_CxCR_NBREQ_Pos) | syncPolarity | ((uint32_t)pSyncConfig->SyncEnable << DMAMUX_CxCR_SE_Pos) | ((uint32_t)pSyncConfig->EventEnable << DMAMUX_CxCR_EGE_Pos));
...
HAL_StatusTypeDef HAL_DMAEx_ConfigMuxRequestGenerator(DMA_HandleTypeDef *hdma,
	...
	{
		assert_param(
				IS_DMA_DMAMUX_REQUEST_GEN_SIGNAL_ID(pRequestGeneratorConfig->SignalID));
	} else {
		assert_param(
				IS_BDMA_DMAMUX_REQUEST_GEN_SIGNAL_ID(pRequestGeneratorConfig->SignalID));
	}
