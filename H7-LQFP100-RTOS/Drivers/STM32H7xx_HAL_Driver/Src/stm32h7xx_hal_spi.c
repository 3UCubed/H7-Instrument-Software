...
static HAL_StatusTypeDef SPI_WaitOnFlagUntilTimeout(
		const SPI_HandleTypeDef *hspi, uint32_t Flag, FlagStatus FlagStatus,
...
HAL_StatusTypeDef HAL_SPI_Init(SPI_HandleTypeDef *hspi) {
  ...
  else
  {
	...
	WRITE_REG(hspi->Instance->CFG2,
			(hspi->Init.NSSPMode | hspi->Init.TIMode | hspi->Init.NSSPolarity
					| hspi->Init.NSS | hspi->Init.CLKPolarity
					| hspi->Init.CLKPhase | hspi->Init.FirstBit
					| hspi->Init.Mode | hspi->Init.MasterInterDataIdleness
					| hspi->Init.Direction | hspi->Init.MasterSSIdleness
					| hspi->Init.IOSwap));
...
HAL_StatusTypeDef HAL_SPI_Transmit(SPI_HandleTypeDef *hspi,
	...
	assert_param(
			IS_SPI_DIRECTION_2LINES_OR_1LINE_2LINES_TXONLY(hspi->Init.Direction));
	...
	if (SPI_WaitOnFlagUntilTimeout(hspi, SPI_FLAG_EOT, RESET, Timeout,
			tickstart) != HAL_OK) {
	...
	if (hspi->Init.DataSize > SPI_DATASIZE_16BIT) {
		...
		while ((initial_TxXferCount > 0UL) || (initial_RxXferCount > 0UL)) {
			...
			if ((__HAL_SPI_GET_FLAG(hspi, SPI_FLAG_TXP))
					...
					&& (initial_RxXferCount
							< (initial_TxXferCount + fifo_length))) {
				*((__IO uint32_t*) &hspi->Instance->TXDR) =
						*((const uint32_t*) hspi->pTxBuffPtr);
				hspi->pTxBuffPtr += sizeof(uint32_t);
				hspi->TxXferCount--;
			...
			if (initial_RxXferCount > 0UL) {
				...
				/* Check RXWNE flag if RXP cannot be reached */
				else if ((initial_RxXferCount < init_max_data_in_fifo)
						&& ((temp_sr_reg & SPI_SR_RXWNE_Msk) != 0UL)) {
					*((uint32_t*) hspi->pRxBuffPtr) =
							*((__IO uint32_t*) &hspi->Instance->RXDR);
					hspi->pRxBuffPtr += sizeof(uint32_t);
	...
	/* Transmit and Receive data in 16 Bit mode */
        ...
        *((__IO uint16_t *)&hspi->Instance->TXDR) = *((const uint16_t *)hspi->pTxBuffPtr);
          ...
          *((uint16_t *)hspi->pRxBuffPtr) = *((__IO uint16_t *)&hspi->Instance->RXDR);
				...
				/* Check RXPLVL flags when RXWNE cannot be reached */
				else if ((initial_RxXferCount == 1UL)
						&& ((temp_sr_reg & SPI_SR_RXPLVL_0) != 0UL)) {
#if defined (__GNUC__)
					*((uint16_t*) hspi->pRxBuffPtr) = *prxdr_16bits;
#else
	...
	/* Transmit and Receive data in 8 Bit mode */
		...
		while ((initial_TxXferCount > 0UL) || (initial_RxXferCount > 0UL)) {
			...
			if (initial_RxXferCount > 0UL) {
				...
				/* Check RXPLVL flags when RXWNE cannot be reached */
				else if ((initial_RxXferCount < 4UL)
						&& ((temp_sr_reg & SPI_SR_RXPLVL_Msk) != 0UL)) {
					*((uint8_t*) hspi->pRxBuffPtr) =
							*((__IO uint8_t*) &hspi->Instance->RXDR);
					hspi->pRxBuffPtr += sizeof(uint8_t);
...
HAL_StatusTypeDef HAL_SPI_Transmit_IT(SPI_HandleTypeDef *hspi,
  ...
  hspi->Reload.TxXferSize  = NULL;
	...
	__HAL_SPI_ENABLE_IT(hspi,
			(SPI_IT_EOT | SPI_IT_TXP | SPI_IT_UDR | SPI_IT_FRE | SPI_IT_MODF | SPI_IT_TSERF));
...
HAL_StatusTypeDef HAL_SPI_TransmitReceive_IT(SPI_HandleTypeDef *hspi,
  ...
  hspi->Reload.TxXferSize  = NULL;
	...
	__HAL_SPI_ENABLE_IT(hspi,
			(SPI_IT_EOT | SPI_IT_DXP | SPI_IT_UDR | SPI_IT_OVR | SPI_IT_FRE | SPI_IT_MODF | SPI_IT_TSERF));
...
HAL_StatusTypeDef HAL_SPI_Receive_DMA(SPI_HandleTypeDef *hspi, uint8_t *pData,
	...
	if (HAL_OK
			!= HAL_DMA_Start_IT(hspi->hdmarx, (uint32_t) &hspi->Instance->RXDR,
...
HAL_StatusTypeDef HAL_SPI_TransmitReceive_DMA(SPI_HandleTypeDef *hspi,
	...
	/* Enable the SPI Error Interrupt Bit */
	__HAL_SPI_ENABLE_IT(hspi,
			(SPI_IT_OVR | SPI_IT_UDR | SPI_IT_FRE | SPI_IT_MODF));
...
void HAL_SPI_IRQHandler(SPI_HandleTypeDef *hspi) {
	...
	/* SPI in SUSPEND mode  ----------------------------------------------------*/
	if (HAL_IS_BIT_SET(itflag,
			SPI_FLAG_SUSP) && HAL_IS_BIT_SET(itsource, SPI_FLAG_EOT)) {
	...
	if (HAL_IS_BIT_CLR(trigger,
			SPI_FLAG_OVR) && HAL_IS_BIT_CLR(trigger, SPI_FLAG_UDR) &&
	...
	if (HAL_IS_BIT_CLR(trigger,
			SPI_FLAG_OVR) && HAL_IS_BIT_SET(trigger, SPI_FLAG_RXP) &&
	...
	if (HAL_IS_BIT_CLR(trigger,
			SPI_FLAG_UDR) && HAL_IS_BIT_SET(trigger, SPI_FLAG_TXP) &&
			HAL_IS_BIT_CLR(trigger, SPI_FLAG_DXP)) {
		hspi->TxISR(hspi);
		handled = 1UL;
	}
  ...
  if (HAL_IS_BIT_SET(trigger, SPI_FLAG_TSERF))
  {
	...
	if (HAL_IS_BIT_SET(trigger, SPI_FLAG_EOT)) {
		...
		if (HAL_IS_BIT_CLR(hspi->Instance->CFG1,
...
{
	SPI_HandleTypeDef *hspi =
			(SPI_HandleTypeDef*) ((DMA_HandleTypeDef*) hdma)->Parent; /* Derogation MISRAC2012-Rule-8.13 */
 ...
 */
static HAL_StatusTypeDef SPI_WaitOnFlagUntilTimeout(
		const SPI_HandleTypeDef *hspi, uint32_t Flag, FlagStatus Status,
		uint32_t Timeout, uint32_t Tickstart) {
