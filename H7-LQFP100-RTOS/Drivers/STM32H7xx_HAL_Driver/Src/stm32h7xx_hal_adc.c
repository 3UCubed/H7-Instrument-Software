...
#else
				assert_param(
						IS_ADC_OVERSAMPLING_RATIO(hadc->Init.Oversampling.Ratio));
#endif
				assert_param(
...
HAL_StatusTypeDef HAL_ADC_DeInit(ADC_HandleTypeDef *hadc) {
	...
	__HAL_ADC_DISABLE_IT(hadc,
			(ADC_IT_AWD3 | ADC_IT_AWD2 | ADC_IT_AWD1 | ADC_IT_JQOVF | ADC_IT_OVR | ADC_IT_JEOS | ADC_IT_JEOC | ADC_IT_EOS | ADC_IT_EOC | ADC_IT_EOSMP | ADC_IT_RDY));

	/* Reset register ISR */
	__HAL_ADC_CLEAR_FLAG(hadc,
			(ADC_FLAG_AWD3 | ADC_FLAG_AWD2 | ADC_FLAG_AWD1 | ADC_FLAG_JQOVF | ADC_FLAG_OVR | ADC_FLAG_JEOS | ADC_FLAG_JEOC | ADC_FLAG_EOS | ADC_FLAG_EOC | ADC_FLAG_EOSMP | ADC_FLAG_RDY));
	...
	/* Reset register CFGR */
	CLEAR_BIT(hadc->Instance->CFGR,
			ADC_CFGR_AWD1CH | ADC_CFGR_JAUTO | ADC_CFGR_JAWD1EN | ADC_CFGR_AWD1EN | ADC_CFGR_AWD1SGL | ADC_CFGR_JQM | ADC_CFGR_JDISCEN | ADC_CFGR_DISCNUM | ADC_CFGR_DISCEN | ADC_CFGR_AUTDLY | ADC_CFGR_CONT | ADC_CFGR_OVRMOD | ADC_CFGR_EXTEN | ADC_CFGR_EXTSEL | ADC_CFGR_RES | ADC_CFGR_DMNGT);
  ...
  {  
	...
	CLEAR_BIT(hadc->Instance->OFR1,
			ADC_OFR1_SSATE | ADC_OFR1_OFFSET1_CH | ADC_OFR1_OFFSET1);
	/* Reset register OFR2 */
	CLEAR_BIT(hadc->Instance->OFR2,
			ADC_OFR2_SSATE | ADC_OFR2_OFFSET2_CH | ADC_OFR2_OFFSET2);
	/* Reset register OFR3 */
	CLEAR_BIT(hadc->Instance->OFR3,
			ADC_OFR3_SSATE | ADC_OFR3_OFFSET3_CH | ADC_OFR3_OFFSET3);
	/* Reset register OFR4 */
	CLEAR_BIT(hadc->Instance->OFR4,
			ADC_OFR4_SSATE | ADC_OFR4_OFFSET4_CH | ADC_OFR4_OFFSET4);
			...
			if ((READ_BIT(tmp_cfgr, ADC_CFGR_JAUTO) == 0UL)
				...
				if (__HAL_ADC_GET_FLAG(hadc, ADC_FLAG_JEOS)) {
					...
					if (READ_BIT(tmp_cfgr, ADC_CFGR_JQM) == 0UL) {
						...
						if (LL_ADC_INJ_IsConversionOngoing(hadc->Instance)
								== 0UL) {
							/* Disable ADC end of sequence conversion interrupt  */
							__HAL_ADC_DISABLE_IT(hadc,
...
HAL_StatusTypeDef HAL_ADC_ConfigChannel(ADC_HandleTypeDef *hadc,
	...
	if (hadc->Init.OversamplingMode == ENABLE) {
		assert_param(
				IS_ADC_RANGE(ADC_GET_RESOLUTION(hadc), sConfig->Offset / (hadc->Init.Oversampling.Ratio + 1U)));
	} else {
#if defined(ADC_VER_V5_V90)
    if (hadc->Instance == ADC3)
    {
      assert_param(IS_ADC3_RANGE(ADC_GET_RESOLUTION(hadc), sConfig->Offset));
		...
		{
			assert_param(
					IS_ADC_RANGE(ADC_GET_RESOLUTION(hadc), sConfig->Offset));
		}
	}
...
HAL_StatusTypeDef HAL_ADC_AnalogWDGConfig(ADC_HandleTypeDef *hadc,
	...
	assert_param(IS_ADC_ALL_INSTANCE(hadc->Instance));
	assert_param(
			IS_ADC_ANALOG_WATCHDOG_NUMBER(AnalogWDGConfig->WatchdogNumber));
  ...
  if (hadc->Instance == ADC3)
  {
...
#endif /* ADC_VER_V5_V90 */
	{
		...
		if (hadc->Init.OversamplingMode == ENABLE) {
			/* Case of oversampling enabled: thresholds are compared to oversampling
			 intermediate computation (after ratio, before shift application) */
			assert_param(
					IS_ADC_RANGE(ADC_GET_RESOLUTION(hadc), AnalogWDGConfig->HighThreshold / (hadc->Init.Oversampling.Ratio + 1UL)));
			assert_param(
					IS_ADC_RANGE(ADC_GET_RESOLUTION(hadc), AnalogWDGConfig->LowThreshold / (hadc->Init.Oversampling.Ratio + 1UL)));
		} else {
			/* Verify if thresholds are within the selected ADC resolution */
			assert_param(
	...
	if ((tmp_adc_is_conversion_on_going_regular == 0UL)
		...
		if (AnalogWDGConfig->WatchdogNumber == ADC_ANALOGWATCHDOG_1) {
			...
			tmpAWDLowThresholdShifted = ADC_AWD1THRESHOLD_SHIFT_RESOLUTION(hadc,
...
#else
			MODIFY_REG(hadc->Instance->LTR1, ADC_LTR_LT,
					tmpAWDLowThresholdShifted);
			MODIFY_REG(hadc->Instance->HTR1, ADC_HTR_HT,
          ...
          else
          {
			...
			default: /* ADC_ANALOGWATCHDOG_NONE */
				LL_ADC_SetAnalogWDMonitChannels(hadc->Instance,
						AnalogWDGConfig->WatchdogNumber, LL_ADC_AWD_DISABLE);
				break;
			}
      ...
      else
      {
        ...
        else
        {
          ...
          MODIFY_REG(hadc->Instance->HTR3_RES11,  ADC_HTR_HT, tmpAWDHighThresholdShifted);
			...
			if (AnalogWDGConfig->WatchdogNumber == ADC_ANALOGWATCHDOG_2) {
				/* Set ADC analog watchdog thresholds value of both thresholds high and low */
				MODIFY_REG(hadc->Instance->LTR2, ADC_LTR_LT,
						tmpAWDLowThresholdShifted);
				MODIFY_REG(hadc->Instance->HTR2, ADC_HTR_HT,
						tmpAWDHighThresholdShifted);
			} else {
				/* Set ADC analog watchdog thresholds value of both thresholds high and low */
				MODIFY_REG(hadc->Instance->LTR3, ADC_LTR_LT,
						tmpAWDLowThresholdShifted);
				MODIFY_REG(hadc->Instance->HTR3, ADC_HTR_HT,
						tmpAWDHighThresholdShifted);
			}
